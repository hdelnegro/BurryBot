"""
kalshi_agent/paper_trader.py — Live paper trading loop for Kalshi.

"Paper trading" means:
  - We fetch REAL, LIVE prices from Kalshi every few minutes.
...
from data_fetcher import fetch_markets, fetch_price_history, fetch_latest_price
...
class PaperTrader:
    """
    Runs a strategy in paper-trading mode: real Kalshi prices, simulated trades.
    """
    ...
    def run(self) -> dict:
        ...
        print(f"\nPaper Trading Mode — {self.strategy.name} [Kalshi]")
        ...
        while datetime.utcnow() < end_time and not _stop_requested:
            if self.tick_count > 0 and self.tick_count % MARKET_REFRESH_INTERVAL_TICKS == 0:
                print("\n[Market Refresh] Re-fetching Kalshi market list...")
                sys.stdout.flush()
    ...
    def _refresh_markets(self, initial: bool = False) -> None:
        ...
        if initial:
            ...
            print(f"Watching {len(self.markets)} Kalshi markets:")
    ...
    def _load_history_for_markets(self, markets: List[Market]) -> None:
        """Load initial hourly price history for a list of Kalshi markets."""
        for market in markets:
            ticker = market.yes_token_id
            if ticker in self._known_token_ids:
                continue

            # event_ticker = parent series (first part of ticker before last numeric run)
            import re
            event_ticker = re.sub(r"-\d+$", "", ticker) or ticker

            bars = fetch_price_history(ticker, event_ticker)

            self.price_history.setdefault(ticker, [])
            existing_ts = {b.timestamp for b in self.price_history[ticker]}
            new_bars    = [b for b in bars if b.timestamp not in existing_ts]
            self.price_history[ticker].extend(new_bars)
            self.price_history[ticker].sort(key=lambda b: b.timestamp)

            n   = len(self.price_history[ticker])
            tag = "(new)" if ticker not in self._known_token_ids else "(updated)"
            print(f"  {market.slug[:45]:45s} {tag}: {n} bars")
            sys.stdout.flush()

            self._known_token_ids.add(ticker)
    ...
    def _run_tick(self) -> None:
        ...
        for market in self.markets:
            ticker     = market.yes_token_id
            latest_bar = self._fetch_latest_price(ticker)

            existing = self.price_history.get(ticker, [])

            if latest_bar is None:
                if existing:
                    current_prices[ticker] = existing[-1].price
            ...
            current_prices[ticker] = latest_bar.price
    ...
    def _fetch_latest_price(self, ticker: str) -> Optional[PriceBar]:
        """
        Fetch the most recent price for a Kalshi market.
        Returns None if the new bar is not newer than the last known bar.
        """
        bar = fetch_latest_price(ticker)
        if bar is None:
            return None

        existing = self.price_history.get(ticker, [])
        if existing and bar.timestamp <= existing[-1].timestamp:
